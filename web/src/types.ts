...
export type Interaction = { id: string; upstreamId: string; protocol: "http"|"redis"|"websocket"|"mysql"; connection?: string; operation: string; startedAt: string; durationUs: number; outcome: "ok"|"error"; error?: string; request: Payload; response: Payload; attributes?: Record<string,string> };
...
export type Upstream = { id: string; name: string; protocol: "http"|"redis"|"mysql"; listenAddr: string; target: string; enabled: boolean; listenerTls?: ListenerTLSOptions; http?: HTTPOptions; redis?: RedisOptions; mysql?: MySQLOptions };
