...
func TestValidateKeepsProtocolSpecificTargetsDistinct(t *testing.T) {
	cases := []struct {
		...
		item  Upstream
		valid bool
	}{{"http", Upstream{Name: "api", Protocol: "http", ListenAddr: "127.0.0.1:9000", Target: "http://localhost:3000"}, true}, {"elasticsearch", Upstream{Name: "search", Protocol: "elasticsearch", ListenAddr: "127.0.0.1:9201", Target: "http://localhost:9200", HTTP: &HTTPOptions{}}, true}, {"elasticsearch bare target", Upstream{Name: "search", Protocol: "elasticsearch", ListenAddr: "127.0.0.1:9201", Target: "localhost:9200", HTTP: &HTTPOptions{}}, false}, {"grpc h2c", Upstream{Name: "rpc", Protocol: "grpc", ListenAddr: "127.0.0.1:50052", Target: "h2c://localhost:50051", HTTP: &HTTPOptions{}, GRPC: &GRPCOptions{}}, true}, {"grpc https", Upstream{Name: "rpc", Protocol: "grpc", ListenAddr: "127.0.0.1:50052", Target: "https://localhost:50051", HTTP: &HTTPOptions{UpstreamTLS: ClientTLSOptions{Enabled: true}}, GRPC: &GRPCOptions{}}, true}, {"grpc over http1 target", Upstream{Name: "rpc", Protocol: "grpc", ListenAddr: "127.0.0.1:50052", Target: "http://localhost:50051", GRPC: &GRPCOptions{}}, false}, {"grpc without settings", Upstream{Name: "rpc", Protocol: "grpc", ListenAddr: "127.0.0.1:50052", Target: "h2c://localhost:50051"}, false}, {"redis", Upstream{Name: "cache", Protocol: "redis", ListenAddr: "127.0.0.1:6380", Target: "localhost:6379"}, true}, {"mysql", Upstream{Name: "database", Protocol: "mysql", ListenAddr: "127.0.0.1:3307", Target: "localhost:3306", MySQL: &MySQLOptions{ListenerUsername: "app", ListenerPassword: "proxy-secret", UpstreamUsername: "db"}}, true}, {"mysql without credentials", Upstream{Name: "database", Protocol: "mysql", ListenAddr: "127.0.0.1:3307", Target: "localhost:3306", MySQL: &MySQLOptions{}}, false}, {"postgres", Upstream{Name: "database", Protocol: "postgres", ListenAddr: "127.0.0.1:5433", Target: "localhost:5432", Postgres: &PostgresOptions{ListenerUsername: "app", ListenerPassword: "proxy-secret", UpstreamUsername: "db", Database: "app"}}, true}, {"postgres without credentials", Upstream{Name: "database", Protocol: "postgres", ListenAddr: "127.0.0.1:5433", Target: "localhost:5432", Postgres: &PostgresOptions{}}, false}, {"mongodb", Upstream{Name: "documents", Protocol: "mongodb", ListenAddr: "127.0.0.1:27018", Target: "localhost:27017", MongoDB: &MongoDBOptions{ListenerUsername: "proxy", ListenerPassword: "listener-secret", UpstreamUsername: "app", UpstreamPassword: "upstream-secret"}}, true}, {"mongodb bad auth", Upstream{Name: "documents", Protocol: "mongodb", ListenAddr: "127.0.0.1:27018", Target: "localhost:27017", MongoDB: &MongoDBOptions{ListenerUsername: "proxy"}}, false}, {"http target on redis", Upstream{Name: "cache", Protocol: "redis", ListenAddr: "127.0.0.1:6380", Target: "http://localhost:6379"}, false}, {"bare target on http", Upstream{Name: "api", Protocol: "http", ListenAddr: "127.0.0.1:9000", Target: "localhost:3000"}, false}}
...
func TestSecretsAreWriteOnlyAndPreservedOnEdit(t *testing.T) {
	...
	mongoOriginal := Upstream{ID: "mongo", Name: "mongo", Protocol: "mongodb", ListenAddr: "127.0.0.1:27018", Target: "localhost:27017", MongoDB: &MongoDBOptions{ListenerUsername: "proxy", ListenerPassword: "listener-secret", UpstreamUsername: "app", UpstreamPassword: "database-secret"}}
	mongoPublic := PublicUpstream(mongoOriginal)
	if mongoPublic.MongoDB.ListenerPassword != "" || !mongoPublic.MongoDB.ListenerPasswordSet || mongoPublic.MongoDB.UpstreamPassword != "" || !mongoPublic.MongoDB.UpstreamPasswordSet {
		t.Fatalf("public MongoDB credentials leaked or lost state: %#v", mongoPublic.MongoDB)
	}
	mongoMerged := MergeSecrets(mongoPublic, mongoOriginal)
