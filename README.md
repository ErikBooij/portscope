...
Portscope currently has eight real protocol adapters:
...
- **gRPC:** native HTTP/2 and h2c forwarding; incremental five-byte envelope parsing across arbitrary DATA-frame boundaries; unary, client-streaming, server-streaming, and bidirectional-streaming call/message observations; gzip message decompression; trailers, canonical status, status-message, deadline, and status-detail inspection; optional descriptor-set-driven protobuf-to-JSON decoding; TLS and mutual TLS on either leg; metadata injection; and bounded capture that never buffers an entire oversized message.
...
For gRPC, choose the gRPC upstream type and use an `h2c://` target for cleartext HTTP/2 or `https://` for TLS. The listener accepts h2c by default or HTTP/2 over TLS when listener TLS is configured. Calls, messages, compression, status, trailers, and deadlines are inspected without a schema. To render protobuf bodies as JSON, generate a self-contained descriptor set with `protoc --include_imports --descriptor_set_out=service.protoset ...` and configure its local path. Header policies operate on gRPC metadata and can inject write-only authorization values.
...
The key design is documented in [docs/architecture.md](docs/architecture.md). Protocol adapters share lifecycle and the observation envelope, not parser or session semantics. Exact compatibility contracts are in [docs/mysql-compatibility.md](docs/mysql-compatibility.md), [docs/postgres-compatibility.md](docs/postgres-compatibility.md), [docs/search-compatibility.md](docs/search-compatibility.md), [docs/mongodb-compatibility.md](docs/mongodb-compatibility.md), [docs/rabbitmq-compatibility.md](docs/rabbitmq-compatibility.md), and [docs/grpc-compatibility.md](docs/grpc-compatibility.md).

Current honest limits:

- HTTP CONNECT tunnels and RFC 8441 WebSockets over HTTP/2 extended CONNECT are not implemented. Classic RFC 6455 upgrades are supported over HTTP/1.1, including TLS on either side. Negotiated WebSocket extensions pass through unchanged; extension-transformed payloads such as per-message-deflate are captured as bytes rather than decompressed.
- gRPC schema discovery is intentionally explicit: server reflection calls pass through and are visible, but Portscope does not issue hidden reflection RPCs. Supply a descriptor set for JSON rendering. `identity` and `gzip` message encodings are decoded; other encodings remain correctly forwarded and appear as bounded opaque messages. gRPC-Web and HTTP/1.x gRPC are rejected rather than mislabeled as native gRPC.
