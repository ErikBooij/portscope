...
Authorization, proxy-authorization, cookie, and set-cookie HTTP headers are redacted before persistence. Configured sensitive header rules extend that set. Redis listener authentication is terminated before an upstream connection is authenticated; `AUTH` and `HELLO ... AUTH` payloads are redacted, and the latter is rewritten without its listener credentials. MySQL authentication packets are never captured as payloads. Both Redis and MySQL have separate listener/upstream write-only password settings. Persisted secrets are write-only at the API seam: a public configuration carries only a `valueSet`/`passwordSet` marker, and edit requests merge unchanged secrets inside the store module.

TLS construction is another shared deep module used by all three protocol adapters. It owns the TLS 1.2 floor, system/custom trust roots, SNI overrides, client certificates, listener certificates, client-CA verification, and ALPN. HTTP-specific protocol selection, WebSocket upgrade/frame handling, Redis authentication/database state, and MySQL handshakes remain inside their adapters.
